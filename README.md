...
Probabilistic programming with Numpy on GPUs powered by [JAX](https://github.com/google/jax) for autograd and JIT support. 

## What is NumPyro?
 
NumPyro is a small probabilistic programming library built on [JAX](https://github.com/google/jax). It essentially provides a NumPy backend for [Pyro](https://github.com/pyro-ppl/pyro), with some minor changes to the inference API and syntax. Since we use JAX, we get autograd and JIT compilation to GPUs for free. This is an alpha release, and the API is subject to change as the design evolves.
...
For some examples on specifying models and doing inference in NumPyro:
 ...
 - [Bayesian Regression in Numpyro](https://github.com/pyro-ppl/numpyro/blob/master/notebooks/bayesian_regression.ipynb) - Start here to get acquainted with writing a simple model in NumPyro, MCMC inference API, effect handlers and writing custom inference utilities.
 - [Time Series Forecasting](https://github.com/pyro-ppl/numpyro/blob/master/notebooks/time_series_forecasting.ipynb) - Illustrates how to convert for loops in the model to JAX's `lax.scan` primitive for fast inference.
...
In the near term, we plan to work on the following. Please open new issues for feature requests and enhancements:
 ...
 - Other areas motivated by Pyro's research goals and applications, and, interest from the community.
