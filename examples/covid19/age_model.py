...
def countries_log_dens(
    ...
    init_A: np.int64,  # 1D
    # A_AD: np.int64,  # 1D
    dataByAge_mask: np.bool,  # 3D: N2 x A x M_AD
...
def transform_data(data):  # lines 438 -> 503
    ...
    data["init_A"] -= 1
    data["epidemicStart"] -= 1
    data["dataByAgestart"] -= 1
    data["wkend_idx"] -= 1
    data["school_case_time_idx"] -= 1
    ...
    for m, (start, byAge_start, end) in enumerate(zip(
            data["epidemicStart"], data["dataByAgestart"][data["map_country"][:, 1]], data["N"])):
