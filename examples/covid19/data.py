...
def generate_init_values(data, seed=0):
    np.random.seed(seed)
    values = {}
    values["R0"] = 2.66 + np.random.randn(data["M"]) * 0.2
    assert (values["R0"] > 0).all()
    values["log_ifr_age_rnde_mid1"] = np.repeat(0.01, data["M"])
    values["log_ifr_age_rnde_mid2"] = np.repeat(0.01, data["M"])
    values["log_ifr_age_rnde_old"] = np.repeat(0.01, data["M"])
    values["dip_rnde"] = np.random.randn(data["M"]) * 0.005
    values["timeeff_shift_mid1"] = np.repeat(0.01, data["M"])
    values["hyper_timeeff_shift_mid1"] = 100.
    values["upswing_timeeff_reduced"] = np.full((data["N_IMP"], data["M"]), 0.01)
    return values
