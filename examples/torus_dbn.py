...
from numpyro.handlers import block, mask, replay, trace
from numpyro.infer import ELBO, SVI, init_to_sample
...
def torus_dbn(
    ...
    prior_conc=0.1,
    prior_loc=0.0,
    prior_length_shape=100.0,
    prior_length_rate=100.0,
    prior_kappa_min=10.0,
    prior_kappa_max=1000.0,
):
    ...
    transition_probs = numpyro.sample(
        "transition_probs",
        dist.Dirichlet(
            jnp.ones((num_states, num_states), dtype="float32") * num_states
        ).to_event(1),
    )
    ...
    phi_locs = numpyro.sample(
        "phi_locs",
        dist.VonMises(
            jnp.ones(num_states, dtype="float32") * prior_loc,
            jnp.ones(num_states, dtype="float32") * prior_conc,
        ).to_event(1),
    )
    phi_kappas = numpyro.sample(
        "phi_kappas",
        dist.Uniform(
            jnp.ones(num_states, dtype="float32") * prior_kappa_min,
            jnp.ones(num_states, dtype="float32") * prior_kappa_max,
        ).to_event(1),
    )
    psi_locs = numpyro.sample(
        "psi_locs",
        dist.VonMises(
            jnp.ones(num_states, dtype="float32") * prior_loc,
            jnp.ones(num_states, dtype="float32") * prior_conc,
    ...
    with numpyro.plate("sequences", num_sequences, dim=-2):
        if lengths is not None:
            ...
            obs_length = None
        sam_lengths = numpyro.sample(
            "length",
            dist.TransformedDistribution(
                dist.GammaPoisson(length_shape, length_rate), AffineTransform(1.0, 0.0)
            ),
            obs=obs_length,
        )
        if lengths is None:
    ...
    def transition_fn(carry, y):
        ...
        with numpyro.plate("sequences", num_sequences, dim=-2):
            with mask(mask=(t < lengths)[..., None]):
                state = numpyro.sample(
                    "state", dist.Categorical(transition_probs[prev_state])
                )
                if phi is not None:
                    obs_phi = phi[..., None]
                else:
                ...
                with numpyro.plate("elements", 1, dim=-1):
                    numpyro.sample(
                        "phi",
                        dist.VonMises(phi_locs[state], phi_kappas[state]),
                        obs=obs_phi,
                    )
                    numpyro.sample(
                        "psi",
                        dist.VonMises(psi_locs[state], psi_kappas[state]),
                        obs=obs_psi,
                    )
...
def sample_and_plot(model, guide, filename=None, num_sequences=128, num_states=5):
    guide_trace = trace(guide).get_trace(
        num_sequences=num_sequences, num_states=num_states
    )
    samples = infer_discrete(
        trace(replay(model, guide_trace)).get_trace, first_available_dim=-3
    )(num_sequences=num_sequences, num_states=num_states)
    lengths = samples["length"]["value"][..., 0].int()
    phis = jnp.concatenate(
        [site["value"] for name, site in samples if re.match(r"phi_\d", name)], dim=-1
    )
    psis = jnp.cat(
        [site["value"] for name, site in samples if re.match(r"psi_\d", name)], dim=-1
    )
...
def plot_rama(lengths, phis, psis, filename="rama", dir="figs"):
    ...
    ax.hexbin(
        np.concatenate(
            [phiseq[: lengths[t]].flatten() for t, phiseq in enumerate(np.array(phis))]
        ),
        np.concatenate(
            [psiseq[: lengths[t]].flatten() for t, psiseq in enumerate(np.array(psis))]
        ),
        bins="log",
    )
    multiple = Multiple()
...
DataLoaderState = namedtuple(
    "DataLoaderState", ("iteration", "rng_key", "indexes", "max_iter")
)
...
def dataloader(*xs, batch_size=32, train_size=None, test_size=None, shuffle=True):
    ...
    max_train_iter, max_test_iter = (
        len(trainxs[0]) // batch_size,
        len(testxs[0]) // batch_size,
    )
    ...
    def make_dataset(dxs, max_iter):
        ...
        def next_step(state):
            ...
            batch = tuple(
                x[state.indexes[iteration * batch_size : (iteration + 1) * batch_size]]
                for x in dxs
            )
            if iteration + 1 == state.max_iter:
                ...
                return batch, DataLoaderState(
                    state.iteration + 1, rng_key, indexes, state.max_iter
                )
            else:
                return batch, DataLoaderState(
                    state.iteration + 1, state.rng_key, state.indexes, state.max_iter
                )
...
def main(_argv):
    ...
    blocked_model = block(
        ...
        and site["name"].startswith("state"),
    )
    ...
    (init_train, next_train), _ = dataloader(
        phis, psis, lengths, train_size=0.9, batch_size=batch_size
    )
    total_losses = []
        ...
        for i in pbar:
            ...
            for j in range(data_state.max_iter):
                ...
                pbar.set_description_str(
                    f"SVI (batch {j}/{data_state.max_iter}):"
            ...
            if i % plot_rate == 0:
                sample_and_plot(
                    ...
                    num_sequences=data_state.max_iter * batch_size,
