...
def main(args):
    ...
    ax1.set(xlim=(-20, 20), ylim=(-9, 9), ylabel="y", title="Funnel samples with centered parameterization")

    ax2.plot(reparam_samples["x"][:, 0], reparam_samples["y"], "go", alpha=0.3)
    ax2.set(xlim=(-20, 20), ylim=(-9, 9), xlabel="x[0]", ylabel="y", title="Funnel samples with non-centered parameterization")
