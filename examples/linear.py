...
from data import get_data_linear
from vjp import pcpcg_quad_form_log_det2_linear


def kernel(X, Z, eta1, c):
...
def gaussian_model(X, Y, hypers, method="direct", num_probes=1, cg_tol=0.001):
    ...
    eta1, kappa = sample_hypers(sigma, S, N, P, hypers)
    ...
    if method != 'ppcg':
        jitter = 1.0e-4
        k_sigma = kernel(kX, kX, eta1, hypers['c']) + (jitter + sigma ** 2) * np.eye(N)
    ...
    if method == "direct":
        ...
        probe = sample_aux_noise(shape=(num_probes, N))
        subsample = sample_permutation(N)
        #subsample = sample_permutation(N)[:N // 10]
        qfld, res_norm, cg_iters = pcpcg_quad_form_log_det2_linear(kappa,
             Y, eta1, sigma, hypers['c'], X, probe, rank, cg_tol, max_iters, dilation, subsample)
...
def do_svi(model, guide, args, rng_key, X, Y, hypers, num_samples=4):
    ...
    return_sites = ['eta1', 'kappa', 'omega', 'lambda']
...
def main(args):
    ...
    X, Y, expected_thetas = get_data_linear(N=N, P=P,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Linear Interaction Discovery")
    ...
    parser.add_argument("--num-data", default=1500, type=int)
    ...
    parser.add_argument("--active-dimensions", default=4, type=int)
    parser.add_argument("--thinning", default=10, type=int)
    parser.add_argument("--device", default='gpu', type=str, help='use "cpu" or "gpu".')
    parser.add_argument("--likelihood", default='gaussian', type=str, choices=['bernoulli', 'gaussian'])
