...
def matern32_kernel(X, Z, var=1., length=.5, jitter=1.0e-6):
    d = jnp.sqrt(.5) * jnp.sqrt(jnp.power((X[:, None] - Z), 2.0)) / length
...
def model(X, Y, kernel=matern32_kernel):
    ...
    var = numpyro.sample("var", dist.LogNormal(0., 1.))
...
class GP:
    ...
    def fit(self, X, Y, rng_key, n_step):
        ...
        svi = SVI(model, guide=AutoDelta(model), optim=optim, loss=Trace_ELBO(),
                  X=X, Y=(Y - self.y_mean) / self.y_std)
    ...
    def predict(self, X, return_std=False):
        ...
        k_pX = self.kernel(X, self.X_train, **self.kernel_params, jitter=0.)
        ...
        if return_std:
            return (mean * self.y_std) + self.y_mean, jnp.sqrt(jnp.diag(K * self.y_std ** 2))
        else:
...
class ThompsonSamplingGP:
    ...
    # initialization
    def __init__(self, gp, n_random_draws, objective, x_bounds, interval_resolution=1000):
        # Gaussian Process
        self.gp = gp
    ...
    def choose_next_sample(self, n_step=2_000):
        ...
        # if we do, we fit the GP and choose the next point based on the posterior draw minimum
            ...
            posterior_mean, posterior_std = self.gp.predict(self.X_grid, return_std=True)
...
def main(args):
    ...
    # do inference
    thompson = ThompsonSamplingGP(gp, n_random_draws=args.num_random, objective=ackley_1d, x_bounds=(-4, 4))

    for i in range(20):
        X, y, X_grid, posterior_sample, posterior_mean, posterior_std = thompson.choose_next_sample(
        ...
        ax.plot(X_grid, posterior_sample, color="red", linestyle="-", label="posterior sample")
        # plot 90% confidence level of predictions
        ax.fill_between(X_grid, posterior_mean - posterior_std, posterior_mean + posterior_std, color="red", alpha=.5)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num-random", nargs="?", default=2, type=int, help="number of random draws")
    parser.add_argument("--num-step", nargs="?", default=2_000, type=int, help="number of steps for optimization")
