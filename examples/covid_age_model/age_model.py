...
def model(data, reparam=False):
    ...
    R0 = jnp.stack([numpyro.sample("R0_{}".format(m), dist.LogNormal(0.98, 0.2)) for m in range(data['M'])])
    e_cases_N0 = jnp.stack([numpyro.sample("e_cases_N0_{}".format(m), dist.LogNormal(4.85, 0.4)) for m in range(data['M'])])

    reparam_config = {"dip_rnde_{}".format(m): TransformReparam() for m in range(data['M'])} if reparam else {}
    with numpyro.handlers.reparam(config=reparam_config):
        dip_rnde = jnp.stack([numpyro.sample("dip_rnde_{}".format(m), dist.TransformedDistribution(
                                             dist.Normal(0., 1.), AffineTransform(0., sd_dip_rnde))) for m in range(data['M'])])

    assert R0.shape == (data["M"],)
    assert e_cases_N0.shape == (data["M"],)
    assert dip_rnde.shape == (data["M"],)
