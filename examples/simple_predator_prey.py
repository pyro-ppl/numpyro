...
def model(prey, predator, idxs):
    ...
    sprey, spredator = predator_prey(prey0, predator0, r=r, k=k, s=s, a=a, u=u, v=v)
    with numpyro.plate("data", prey.shape[0], dim=-2):
        numpyro.sample("prey_obs", dist.Normal(sprey[idxs], 100.0), obs=prey)
        numpyro.sample(
            "predator_obs", dist.Normal(spredator[idxs], 100.0), obs=predator
        )


if __name__ == "__main__":
    config.update("jax_debug_nans", True)
    prey, predator = predator_prey(50.0, 5.0)
    scale = 3
    idxs = [2, 13, 29, 31, 47]
    ...
    try:
        ...
        grady = jax.grad(
            lambda p: svi.loss.loss(
                rng_key_eval, p, svi.model, svi.guide, obs_prey, obs_predator, idxs
            )
        )
        # For getting the Jaxpr
        grad_jaxpr, consts, trees = _make_jaxpr_with_consts(grady, stage_out=True)(
            params
        )
        print(grad_jaxpr)
