...
predator_prey = runge_kutta_4(predator_prey_step, step_size, num_steps, dampening_rate, 
                              lyapunov_scale, clip, unconstrain_fn=lambda k, v: np.where(v < 10, np.log(np.clip(np.expm1(v), 1e-7, 1e7)), v),
                              constrain_fn=lambda k, v: jax.nn.softplus(v))
...
# %%
svgd = numpyro.infer.SVGD(model, AutoDelta(model, init_strategy=init_with_noise(init_to_median(), 3.0)), 
                          numpyro.optim.Adam(1e-4), numpyro.infer.ELBO(),
