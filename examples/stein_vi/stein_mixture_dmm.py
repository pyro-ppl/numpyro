...
def model(seqs, seqs_rev, lengths, *,
    ...
    masks = np.repeat(np.expand_dims(np.arange(max_seq_length), axis=0), batch_size, axis=0) <            np.expand_dims(lengths, axis=-1)
...
# %%
svgd = SVGD(model, WrappedGuide(guide, reinit_hide_fn=lambda site: site['name'].endswith('$params')), Adam(8e-4), ELBO(), RBFKernel(), num_stein_particles=5)
...
for j in pbar:
    i = j % num_epochs
    seqs, seqs_rev, lengths = get_batch(i, ds_indxs)
    state, loss = svgd.update(state, seqs, seqs_rev, lengths)
