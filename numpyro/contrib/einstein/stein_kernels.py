...
class SteinKernel(ABC):
    ...
    def init(self, rng_key, particles_shape):
        """Initializes the kernel :param rng_key: a JAX PRNGKey to initialize the
        kernel :param tuple particles_shape: shape of the input `particles` in
        :meth:`compute`"""
        pass
...
class MixtureKernel(SteinKernel):
    """Calculates a mixture of multiple kernels from eq. 1 of [1]. The kernel is
    given by.
...
class GraphicalKernel(SteinKernel):
    """Calculates the graphical kernel, also called the coordinate-wise kernel, from
    Theorem 1 in [1]. The kernel is given by.
...
class ProbabilityProductKernel(SteinKernel):
    """**EXPERIMENTAL** Compute the unormalized probability product kernel for
    Gaussians given by eq. 5 in [1].
