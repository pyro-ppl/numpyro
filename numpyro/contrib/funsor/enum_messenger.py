...
class trace(OrigTraceMessenger):
    ...
    def postprocess_message(self, msg):
        ...
        if msg["type"] in ("sample", "param", "markov_chain"):
...
class vectorized_markov(NamedMessenger):
    ...
    @staticmethod
    def _markov_chain(name, markov_vars=set(), suffixes=list()):
        """
        ...
        """
        chain = frozenset(
            {tuple(f"{var}{suffix}" for suffix in suffixes) for var in markov_vars}
        )
        #  if not _PYRO_STACK:
        #      return chain
    ...
    def __iter__(self):
        ...
        for self._suffix in range(self.history):
            ...
            yield self._suffix
        with self:
            with OrigPlateMessenger(
                name=self.name, size=self.size - self.history, dim=self.dim
            ) as time:
                time_indices = [time + i for i in range(self.history + 1)]
                time_slices = [
    ...
    def process_message(self, msg):
        if msg["type"] == "sample":
            ...
            if isinstance(self._suffix, slice):
                ...
                msg["name"] = msg["name"][: -len(str(self._indices))] + str(
                    self._suffix
                )
    ...
    def postprocess_message(self, msg):
        ...
        if msg["type"] == "sample":
            ...
            if str(self._suffix) == str(self._suffixes[-1]):
                ...
                for name in set(msg["infer"].get("name_to_dim", {})) & set(
                    self._auxiliary_to_markov
                ):
