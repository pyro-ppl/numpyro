...
class VonMises(Distribution):
    ...
    def __init__(self, loc, concentration, validate_args=None):
        ...
        super(VonMises, self).__init__(batch_shape=batch_shape, validate_args=validate_args)
    ...
    def log_prob(self, value):
        return -(jnp.log(2 * jnp.pi) + jnp.log(i0e(self.concentration))) + self.concentration * (jnp.cos((value - self.loc) % (2 * jnp.pi)) - 1)

    @property
    def mean(self):
        """ Computes circular mean of distribution. NOTE: same as location when mapped to support [-pi, pi] """
        return jnp.broadcast_to((self.loc + jnp.pi) % (2.0 * jnp.pi) - jnp.pi, self.batch_shape)

    @property
    def variance(self):
        """ Computes circular variance of distribution """
        return jnp.broadcast_to(1.0 - i1e(self.concentration) / i0e(self.concentration), self.batch_shape)
...
class ProjectedNormal(Distribution):
    ...
    def log_prob(self, value):
        ...
        raise NotImplementedError(f"ProjectedNormal.log_prob() is not implemented for dim = {dim}. " "Consider using handlers.reparam with ProjectedNormalReparam.")
...
def _projected_normal_log_prob_2(concentration, value):
    ...
    para_part = jnp.log((jnp.exp((-0.5) * t2) * ((2 / math.pi) ** 0.5) + t * (1 + erf(t * 0.5 ** 0.5))) / 2)
...
def _projected_normal_log_prob_3(concentration, value):
    ...
    para_part = jnp.log(t * jnp.exp((-0.5) * t2) / (2 * math.pi) ** 0.5 + (1 + t2) * (1 + erf(t * 0.5 ** 0.5)) / 2)
