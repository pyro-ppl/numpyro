...
"""Optimizer classes defined here are light wrappers over the corresponding
optimizers sourced from :mod:`jax.example_libraries.optimizers` with an interface
that is better suited for working with NumPyro inference algorithms."""
...
class _NumPyroOptim(object):
    ...
    def eval_and_update(
        ...
        """Performs an optimization step for the objective function `fn`. For most
        optimizers, the update is performed based on the gradient of the objective
        function w.r.t. the current state. However, for some optimizers such as
        :class:`Minimize`, the update is performed by reevaluating the function
        multiple times to get optimal parameters.

        :param fn: an objective function returning a pair where the first item is a
            scalar loss function to be differentiated and the second item is an
            auxiliary output.
        :param state: current optimizer state.
        :param forward_mode_differentiation: boolean flag indicating whether to use
            forward mode differentiation.
        :return: a pair of the output of objective function and the new optimizer
            state.
        """
    ...
    def eval_and_stable_update(
        ...
        or the gradients are not finite, we will not update the input `state` and
        will set the objective output to `nan`.
...
class ClippedAdam(_NumPyroOptim):
    ...
    :param float clip_norm: All gradient values will be clipped between
        `[-clip_norm, clip_norm]`. **Reference:** `A Method for Stochastic
        Optimization`, Diederik P. Kingma, Jimmy Ba https://arxiv.org/abs/1412.6980
    """
