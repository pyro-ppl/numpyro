...
model block by block, and the generic block kernels :class:`CustomGibbs` and
:class:`DiscreteGibbs`.
"""
...
from numpyro.util import cond, identity, is_prng_key
...
class Gibbs(MCMCKernel):
    ...
    **Example**
    ...
    def __init__(self, blocks: Sequence[tuple[MCMCKernel, SitesSpec]]) -> None:
        ...
        self._sites: tuple[tuple[str, ...], ...] | None = None
    ...
    def _resolve_partition(
        ...
        for kernel, spec, names in zip(self._kernels, self._specs, sites):
    ...
    **References:**
    ...
    def init(
        ...
        self._support_sizes_flat = np.asarray(
            ravel_pytree(discrete_support_sizes(model_trace, sites))[0]
        )
        self._enum = any(
            site["type"] == "sample"
    ...
    def sample(
        ...
        """One :func:`~numpyro.infer.gibbs_util.discrete_gibbs_sweep` with the selected proposal."""
