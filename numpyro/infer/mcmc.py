...
class MCMCKernel(ABC):
    """Defines the interface for the Markov transition kernel that is used for :class:`~numpyro.infer.mcmc.MCMC`
    inference.
    ...
    **Example:**
    ...
    def sample_field(self):
        """The attribute of the `state` object passed to :meth:`sample` that denotes the MCMC sample.
    ...
    def default_fields(self):
        ...
        :meth:`MCMC.run() <numpyro.infer.MCMC.run>` is called)."""
    ...
    def get_diagnostics_str(self, state):
        """Given the current `state`, returns the diagnostics string to be added to progress bar for diagnostics
        purpose."""
        return ""
...
class MCMC(object):
    ...
    def post_warmup_state(self):
        """The state before the sampling phase. If this attribute is not None, :meth:`run` will skip the warmup
        phase and start with the state specified in this attribute.
    ...
    def warmup(
        ...
        **kwargs,
        ...
        :meth:`run` method will skip the warmup adaptation phase. To run `warmup` again for the new data, it is
    ...
    def print_summary(self, prob=0.9, exclude_deterministic=True):
        ...
        :param bool exclude_deterministic: whether or not print out the statistics at deterministic sites.
        """
