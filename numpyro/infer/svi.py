    ...
    **Example:**
    ...
    def update(self, svi_state, *args, forward_mode_differentiation=False, **kwargs):
        ...
        :param forward_mode_differentiation: boolean flag indicating whether to use
            forward mode differentiation. Defaults to False.
    ...
    def stable_update(
        ...
        """Similar to :meth:`update` but returns the current state if the the loss
        or the new state contains invalid values.
    ...
    def run(
        ...
        **kwargs,
        ...
        the optimized parameters and the stacked losses at every step. If
        `num_steps` is large, setting `progress_bar=False` can make the run faster.
