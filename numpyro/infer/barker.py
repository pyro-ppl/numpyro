...
BarkerMHState = namedtuple("BarkerMHState", ["i", "z", "potential_energy", "z_grad", "accept_prob", "mean_accept_prob", "adapt_state", "rng_key"])
"""
...
class BarkerMH(MCMCKernel):
    ...
    **Example**
    ...
    def __init__(self, model=None, potential_fn=None, step_size=1.0, adapt_step_size=True, adapt_mass_matrix=True, dense_mass=False, target_accept_prob=0.4, init_strategy=init_to_uniform):
    ...
    def get_diagnostics_str(self, state):
        return "step size {:.2e}. acc. prob={:.2f}".format(state.adapt_state.step_size, state.mean_accept_prob)

    def _init_state(self, rng_key, model_args, model_kwargs, init_params):
        if self._model is not None:
            params_info, potential_fn_gen, self._postprocess_fn, model_trace = initialize_model(rng_key, self._model, dynamic_args=True, init_strategy=self._init_strategy, model_args=model_args, model_kwargs=model_kwargs)
    ...
    def init(self, rng_key, num_warmup, init_params, model_args, model_kwargs):
        ...
        assert rng_key.shape == (2,), "BarkerMH only supports chain_method='parallel' or chain_method='sequential'." " Please put in a feature request if you think it would be useful to be able " "to use BarkerMH in vectorized mode."
        ...
        wa_init, self._wa_update = warmup_adapter(num_warmup, adapt_step_size=self._adapt_step_size, adapt_mass_matrix=self._adapt_mass_matrix, dense_mass=self._dense_mass, target_accept_prob=self._target_accept_prob)
        ...
        init_state = BarkerMHState(jnp.array(0), init_params, pe, grad, jnp.zeros(()), jnp.zeros(()), wa_state, rng_key)
    ...
    def sample(self, state, model_args, model_kwargs):
        ...
        log_accept_ratio = x_pe - y_pe + jnp.sum(softplus(dx_flat * x_grad_flat_scaled) - softplus(-dx_flat * y_grad_flat_scaled))
        accept_prob = jnp.clip(jnp.exp(log_accept_ratio), a_max=1.0)

        x, x_flat, pe, x_grad = jax.lax.cond(random.bernoulli(key_accept, accept_prob), (y, y_flat, y_pe, y_grad), identity, (x, x_flat, x_pe, x_grad), identity)

        # do not update adapt_state after warmup phase
        adapt_state = jax.lax.cond(i < self._num_warmup, (i, accept_prob, (x,), adapt_state), lambda args: self._wa_update(*args), adapt_state, identity)
