...
def param(
    ...
    :mod:`jax.example_libraries.optimizers`. For an example of how `param`
    statements can be used in inference algorithms, refer to
    :class:`~numpyro.infer.SVI`.
...
def deterministic(name: str, value: ArrayLike) -> ArrayLike:
    ...
    :func:`~numpyro.handlers.trace`), so deterministic sites should be side-effect
    free. The use case for deterministic nodes is to record any values in the model
    execution trace.
...
class plate(Messenger):
    ...
    `plate` context manager, `sample` sites will be automatically broadcasted to the
    size of the plate. Additionally, a scale factor might be applied by certain
    inference algorithms if `subsample_size` is specified.
...
def factor(name: str, log_factor: ArrayLike) -> None:
    """Factor statement to add arbitrary log probability factor to a probabilistic
    model.
