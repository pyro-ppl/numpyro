...
from numpyro.distributions.transforms import LowerCholeskyAffine, PermuteTransform, PowerTransform, SoftplusTransform, biject_to
from numpyro.distributions.util import matrix_to_tril_vec, multinomial, signed_stick_breaking_tril, sum_rightmost, vec_to_tril_matrix
...
_DIST_MAP = {
    ...
    dist.MultinomialLogits: lambda logits, total_count: osp.multinomial(n=total_count, p=_to_probs_multinom(logits)),
    ...
    dist.VonMises: lambda loc, conc: osp.vonmises(loc=np.array(loc, dtype=np.float64), kappa=np.array(conc, dtype=np.float64)),
...
CONTINUOUS = [
    ...
    pytest.param(*T(dist.LKJCholesky, 5, jnp.array([0.5, 1.0, 2.0]), "cvine"), marks=pytest.mark.skipif("CI" in os.environ, reason="reduce time for Travis")),
    pytest.param(*T(dist.LKJCholesky, 3, jnp.array([[3.0, 0.6], [0.2, 5.0]]), "onion"), marks=pytest.mark.skipif("CI" in os.environ, reason="reduce time for Travis")),
    ...
    T(dist.MultivariateNormal, jnp.array([1.0, 3.0]), None, jnp.array([[1.0, 0.5], [0.5, 1.0]]), None),
    T(dist.MultivariateNormal, jnp.array([1.0, 3.0]), None, jnp.array([[[1.0, 0.5], [0.5, 1.0]]]), None),
    T(dist.MultivariateNormal, jnp.array([2.0]), None, None, jnp.array([[1.0, 0.0], [0.5, 1.0]])),
    T(dist.MultivariateNormal, jnp.arange(6, dtype=jnp.float32).reshape((3, 2)), None, None, jnp.array([[1.0, 0.0], [0.0, 1.0]])),
    T(dist.MultivariateNormal, 0.0, None, jnp.broadcast_to(jnp.identity(3), (2, 3, 3)), None),
    T(dist.LowRankMultivariateNormal, jnp.zeros(2), jnp.array([[1.0], [0.0]]), jnp.array([1.0, 1.0])),
    T(dist.LowRankMultivariateNormal, jnp.arange(6, dtype=jnp.float32).reshape((2, 3)), jnp.arange(6, dtype=jnp.float32).reshape((3, 2)), jnp.array([1.0, 2.0, 3.0])),
    ...
    T(dist.TruncatedCauchy, jnp.array([-2.0, 2.0]), jnp.array([0.0, 1.0]), jnp.array([[1.0], [2.0]])),
    T(dist.TruncatedNormal, -1.0, 0.0, 1.0),
    T(dist.TruncatedNormal, 1.0, -1.0, jnp.array([1.0, 2.0])),
    T(dist.TruncatedNormal, jnp.array([-2.0, 2.0]), jnp.array([0.0, 1.0]), jnp.array([[1.0], [2.0]])),
    ...
    T(_TruncatedNormal, jnp.array([0.0, 1.0]), jnp.array([[1.0], [2.0]]), None, jnp.array([-2.0, 2.0])),
...
DIRECTIONAL = [T(dist.VonMises, 2.0, 10.0), T(dist.VonMises, 2.0, jnp.array([150.0, 10.0])), T(dist.VonMises, jnp.array([1 / 3 * jnp.pi, -1.0]), jnp.array([20.0, 30.0])), T(dist.ProjectedNormal, jnp.array([0.0, 0.0])), T(dist.ProjectedNormal, jnp.array([[2.0, 3.0]])), T(dist.ProjectedNormal, jnp.array([0.0, 0.0, 0.0])), T(dist.ProjectedNormal, jnp.array([[-1.0, 2.0, 3.0]]))]

DISCRETE = [
    T(dist.BetaBinomial, 2.0, 5.0, 10),
    T(dist.BetaBinomial, jnp.array([2.0, 4.0]), jnp.array([5.0, 3.0]), jnp.array([10, 12])),
...
def gen_values_within_bounds(constraint, size, key=random.PRNGKey(11)):
    ...
    if constraint is constraints.boolean:
        ...
        return signed_stick_breaking_tril(random.uniform(key, size[:-2] + (size[-1] * (size[-1] - 1) // 2,), minval=-1, maxval=1))
    elif constraint is constraints.corr_matrix:
        cholesky = signed_stick_breaking_tril(random.uniform(key, size[:-2] + (size[-1] * (size[-1] - 1) // 2,), minval=-1, maxval=1))
...
def gen_values_outside_bounds(constraint, size, key=random.PRNGKey(11)):
    if constraint is constraints.boolean:
        ...
        return signed_stick_breaking_tril(random.uniform(key, size[:-2] + (size[-1] * (size[-1] - 1) // 2,), minval=-1, maxval=1)) + 1e-2
    elif constraint is constraints.corr_matrix:
        cholesky = 1e-2 + signed_stick_breaking_tril(random.uniform(key, size[:-2] + (size[-1] * (size[-1] - 1) // 2,), minval=-1, maxval=1))
        return jnp.matmul(cholesky, jnp.swapaxes(cholesky, -2, -1))
    elif constraint is constraints.lower_cholesky:
...
def test_sample_gradient(jax_dist, sp_dist, params):
    ...
    gamma_derived_params = {"Gamma": ["concentration"], "Beta": ["concentration1", "concentration0"], "Chi2": ["df"], "Dirichlet": ["concentration"], "InverseGamma": ["concentration"], "LKJ": ["concentration"], "LKJCholesky": ["concentration"], "StudentT": ["df"]}.get(jax_dist.__name__, [])
    ...
    reparameterized_params = [p for p in jax_class.reparametrized_params if p not in gamma_derived_params]
    if not reparameterized_params:
        pytest.skip("{} not reparametrized.".format(jax_class.__name__))

    nonrepara_params_dict = {k: v for k, v in params_dict.items() if k not in reparameterized_params}
    repara_params = tuple(v for k, v in params_dict.items() if k in reparameterized_params)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(dist.Beta, osp.beta, (1.0, 1.0), marks=pytest.mark.xfail(reason="currently, variance of grad of beta sampler is large")),
        pytest.param(dist.StudentT, osp.t, (1.0,), marks=pytest.mark.xfail(reason="currently, variance of grad of t sampler is large")),
    ],
)
...
def test_log_prob(jax_dist, sp_dist, params, prepend_shape, jit):
    ...
    if sp_dist is None:
        if isinstance(jax_dist, (dist.LeftTruncatedDistribution, dist.RightTruncatedDistribution, dist.TwoSidedTruncatedDistribution)):
...
def test_cdf_and_icdf(jax_dist, sp_dist, params):
    ...
    try:
        if d.shape() == ():
            ...
            assert_allclose(jax.vmap(jax.grad(d.cdf))(samples), jnp.exp(d.log_prob(samples)), atol=1e-5, rtol=rtol)
            assert_allclose(jax.vmap(jax.grad(d.icdf))(quantiles), jnp.exp(-d.log_prob(d.icdf(quantiles))), atol=1e-5, rtol=rtol)
...
def test_log_prob_LKJCholesky_uniform(dimension):
    ...
    for i in range(N):
        ...
        cholesky_to_corr_jac = np.linalg.slogdet(jax.jacobian(_tril_cholesky_to_tril_corr)(sample_tril))[1]
        corr_log_prob.append(log_prob - cholesky_to_corr_jac)

    corr_log_prob = jnp.array(corr_log_prob)
    # test if they are constant
    assert_allclose(corr_log_prob, jnp.broadcast_to(corr_log_prob[0], corr_log_prob.shape), rtol=1e-6)
...
def test_log_prob_LKJCholesky(dimension, concentration):
    ...
    corr_cholesky_logdet = biject_to(constraints.corr_cholesky).log_abs_det_jacobian(unconstrained, sample)
...
def test_mean_var(jax_dist, sp_dist, params):
    ...
    if jax_dist in (_TruncatedNormal, dist.LeftTruncatedDistribution, dist.RightTruncatedDistribution, dist.TwoSidedTruncatedDistribution):
    ...
    if sp_dist and not _is_batched_multivariate(d_jax) and jax_dist not in [dist.VonMises]:
        ...
        marginal = dist.Beta(concentration + 0.5 * (dimension - 2), concentration + 0.5 * (dimension - 2))
        # scale statistics due to linear mapping
        marginal_mean = 2 * marginal.mean - 1
        marginal_std = 2 * jnp.sqrt(marginal.variance)
        expected_mean = jnp.broadcast_to(jnp.reshape(marginal_mean, jnp.shape(marginal_mean) + (1, 1)), jnp.shape(marginal_mean) + d_jax.event_shape)
        expected_std = jnp.broadcast_to(jnp.reshape(marginal_std, jnp.shape(marginal_std) + (1, 1)), jnp.shape(marginal_std) + d_jax.event_shape)
...
def test_distribution_constraints(jax_dist, sp_dist, params, prepend_shape):
    ...
    if sp_dist and not _is_batched_multivariate(d) and not (d.event_shape and prepend_shape):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (constraints.corr_cholesky, jnp.array([[[1, 0], [0, 1]], [[1, 0.1], [0, 1]]]), jnp.array([True, False])),  # NB: not lower_triangular
        (constraints.corr_cholesky, jnp.array([[[1, 0], [1, 0]], [[1, 0], [0.5, 0.5]]]), jnp.array([False, False])),  # NB: not positive_diagonal & not unit_norm_row
        (constraints.corr_matrix, jnp.array([[[1, 0], [0, 1]], [[1, 0.1], [0, 1]]]), jnp.array([True, False])),  # NB: not lower_triangular
        (constraints.corr_matrix, jnp.array([[[1, 0], [1, 0]], [[1, 0], [0.5, 0.5]]]), jnp.array([False, False])),  # NB: not unit diagonal
        ...
        (constraints.integer_interval(-3, 5), jnp.array([-5, -3, 0, 1.1, 5, 7]), jnp.array([False, True, True, False, True, False])),
        ...
        (constraints.lower_cholesky, jnp.array([[[1.0, 0.0], [-2.0, -0.1]], [[1.0, 0.1], [2.0, 0.2]]]), jnp.array([False, False])),
        ...
        (constraints.positive_definite, jnp.array([[[2.0, 0.4], [0.3, 2.0]], [[1.0, 0.1], [0.1, 0.0]]]), jnp.array([False, False])),
        ...
        (constraints.simplex, jnp.array([[0.1, 0.3, 0.6], [-0.1, 0.6, 0.5], [0.1, 0.6, 0.5]]), jnp.array([True, False, False])),
        ...
        (constraints.softplus_lower_cholesky, jnp.array([[[1.0, 0.0], [-2.0, -0.1]], [[1.0, 0.1], [2.0, 0.2]]]), jnp.array([False, False])),
...
def test_biject_to(constraint, shape):
    ...
    if len(shape) == event_dim:
        ...
        elif constraint in [constraints.real_vector, constraints.ordered_vector, constraints.positive_ordered_vector]:
        ...
        elif constraint in [constraints.lower_cholesky, constraints.positive_definite, constraints.softplus_lower_cholesky]:
...
@pytest.mark.parametrize("transform, event_shape", [(PermuteTransform(jnp.array([3, 0, 4, 1, 2])), (5,)), (PowerTransform(2.0), ()), (SoftplusTransform(), ()), (LowerCholeskyAffine(jnp.array([1.0, 2.0]), jnp.array([[0.6, 0.0], [1.5, 0.4]])), (2,))])
...
def test_transformed_distribution(batch_shape, prepend_event_shape, sample_shape):
    base_dist = dist.Normal(0, 1).expand(batch_shape + prepend_event_shape + (6,)).to_event(1 + len(prepend_event_shape))
    t1 = transforms.AffineTransform(0, 2)
    t2 = transforms.LowerCholeskyTransform()
...
@pytest.mark.parametrize("transformed_dist", [dist.TransformedDistribution(dist.Normal(jnp.array([2.0, 3.0]), 1.0), transforms.ExpTransform()), dist.TransformedDistribution(dist.Exponential(jnp.ones(2)), [transforms.PowerTransform(0.7), transforms.AffineTransform(0.0, jnp.ones(2) * 3)])])
...
def test_transformed_transformed_distribution():
    loc, scale = -2, 3
    dist1 = dist.TransformedDistribution(dist.Normal(2, 3), transforms.PowerTransform(2.0))
...
@pytest.mark.parametrize("jax_dist, sp_dist, params", CONTINUOUS)
def test_generated_sample_distribution(jax_dist, sp_dist, params, N_sample=100_000, key=random.PRNGKey(11)):
    ...
    if jax_dist not in [dist.Gumbel]:
        pytest.skip("{} sampling method taken from upstream, no need to" "test generated samples.".format(jax_dist.__name__))
...
@pytest.mark.parametrize("extra_event_dims,expand_shape", [(0, (4, 3, 2, 1)), (0, (4, 3, 2, 2)), (1, (5, 4, 3, 2)), (2, (5, 4, 3))])
...
@pytest.mark.parametrize("batch_shape, mask_shape", [((), ()), ((2,), ()), ((), (2,)), ((2,), (2,)), ((4, 2), (1, 2)), ((2,), (4, 2))])
...
@pytest.mark.parametrize("method, arg", [("to_event", 1), ("mask", False), ("expand", [5])])
