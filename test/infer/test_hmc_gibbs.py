...
@pytest.mark.parametrize("kernel, inner_kernel, kwargs", [(MixedHMC, HMC, {"num_discrete_updates": 5}), (DiscreteHMCGibbs, NUTS, {})])
...
@pytest.mark.parametrize("kernel, inner_kernel, kwargs", [(MixedHMC, HMC, {"num_discrete_updates": 6}), (DiscreteHMCGibbs, NUTS, {})])
...
@pytest.mark.parametrize("kernel, inner_kernel, kwargs", [(MixedHMC, HMC, {"num_discrete_updates": 6}), (DiscreteHMCGibbs, NUTS, {"modified": True}), (DiscreteHMCGibbs, NUTS, {"modified": False})])
...
@pytest.mark.parametrize("kernel, inner_kernel, kwargs", [(MixedHMC, HMC, {"num_discrete_updates": 20}), (DiscreteHMCGibbs, NUTS, {})])
...
def test_block_update_partitioning(num_blocks):
    ...
    new_gibbs_sites, new_gibbs_state = numpyro.infer.hmc_gibbs._block_update(plate_sizes, num_blocks, random.PRNGKey(2), gibbs_sites, gibbs_state)
...
def test_taylor_proxy_norm(subsample_size):
    ...
    def taylor_expand_2nd_order(idx, pos):
        return log_prob[idx] + (log_norm_jac[idx] @ pos) + 0.5 * (pos @ log_norm_hessian[idx]) @ pos

    def taylor_expand_2nd_order_sum(pos):
        return log_prob.sum() + log_norm_jac.sum(0) @ pos + 0.5 * pos @ log_norm_hessian.sum(0) @ pos

    for _ in range(5):
        split_key, perturbe_key, rng_key = random.split(rng_key, 3)
        perturbe_params = ref_params + dist.Normal(0.1, 0.1).sample(perturbe_key, ref_params.shape)
        ...
        assert_allclose(actual_proxy_sub["data"], taylor_expand_2nd_order(subsample_idx, perturbe_params - ref_params), rtol=1e-5)
