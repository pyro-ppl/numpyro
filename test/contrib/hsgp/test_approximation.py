...
def test_kernel_approx_squared_matern(
    ...
    """Ensure that the approximation of the matern kernel is accurate, matching the
    exact kernel implementation from sklearn.
...
def test_kernel_approx_periodic(
    ...
    """Ensure that the approximation of the periodic kernel is accurate, matching
    the exact kernel implementation from sklearn.

    Note that the exact kernel implementation is parameterized with respect to the
    period, and the periodicity is w0**(-1). We adjust the input values by dividing
    by 2*pi.
