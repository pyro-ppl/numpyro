...
from numpyro.contrib.callbacks import (
    Checkpoint,
    EarlyStopping, History, Progbar, TerminateOnNaN, ReduceLROnPlateau
)
...
def test_early_stopping(smoothing, patience):
    es = EarlyStopping(patience=patience, smoothing=smoothing, loss_mode='training', min_delta=.5)
    num_steps = 101
    curr_loss = 10.
    ...
    es.on_train_begin({**train_info, **{'loss': 5.}})
    assert es.best_loss == es.curr_loss == train_info['loss']
    ...
    for num_steps in range(num_steps):
        try:
            ...
            train_info['loss'] = train_info['loss'] - .1 * random.normal(key, ())
...
def test_history():
    ...
    tlosses = 10. + jnp.cumsum(random.normal(vkey, (num_steps,)))
...
def test_progbar(capsys):
    ...
    pb = Progbar()
    pb.vi = vi('vi')
    pb.on_train_begin(train_info(10.))
    losses = 10. + np.cumsum(100 * np.random.normal(size=(num_steps,)))
    for i in pb.progbar:
...
def test_terminate_on_nan():
    ...
    try:
        TerminateOnNaN().on_train_step_end(0, train_info(jnp.array([1, 2, 3, 4.523, 43214])))
    except StopIteration:
        pytest.fail()
    try:
        TerminateOnNaN().on_train_step_end(0, train_info(jnp.array([1, 2, jnp.nan, 4.523, 43214])))
        pytest.fail()
    except StopIteration:
        pass
...
def test_reduce_lr_on_plateau():
    schedule = ReduceLROnPlateau(initial_lr=1., patience=2, factor=1e-1, min_lr=1e-5, frequency='step')
    schedule.best_loss = 10.
    for i in range(100):
        schedule._reduce_lr_on_plateau(11.)
